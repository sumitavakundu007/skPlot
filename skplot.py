...
class plot_save:
    ...
    def contour_file(self):
        if len(self.filePaths) >= 1:
            ...
            self.canvas.get_tk_widget().destroy()
            # Set global variables
            self.f = self.func_entry.get()
            self.x_arr, self.y_arr = [], []
            for i in self.filePaths:
                self.x_arr.append(np.loadtxt(i)[:, 0])
                self.y_arr.append(np.loadtxt(i)[:, 1])

            [self.X, self.Y] = np.meshgrid(self.x_arr, self.y_arr)
            x, y = self.X, self.Y
            self.Z = eval(self.f)
            # contour plot
            self.fig = plt.figure(figsize=(3, 3))
            ...
            self.toolbar.update()
            # Destroy entries
            self.func_lbl.destroy()
            self.func_entry.destroy()

    def back_to_contour_file(self):
        # Destroy the window
        self.toolbar.destroy()
        self.canvas.get_tk_widget().destroy()
        # Set global variables
    ...
    def contour_plot(self):
        self.ax = self.fig.add_subplot()
        if len(self.X) == len(self.Z[1]) and len(self.Y) == len(self.Z[0]):
    ...
    def set_xylabel(self):
        if len(self.filePaths) >= 1:
            if len(self.x_arr) > 0:
                ...
                btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_file_labeling())
        ...
        if self.num_cols == 5:
            if len(self.filePaths) >= 1:
                if len(self.x_arr) > 0:
                    ...
                    btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_lp())
                    btn_replot.place(x=72, y=5)
                else:
                    error_box = messagebox.showerror("Instructions", "Please do any kind of plot")
            else:
    ...
    def set_xyrange(self):
        if len(self.filePaths) >= 1:
            if len(self.x_arr) > 0:
                ...
                btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_file_range())
                btn_replot.place(x=72, y=5)
            else:
                error_box = messagebox.showerror("Instructions", "Please do any kind of plot")
        else:
    ...
    def set_figsize(self):
        if len(self.filePaths) >= 1:
            if len(self.x_arr) > 0:
                ...
                # Replot button
                btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_figsize())
                btn_replot.place(x=72, y=5)
            else:
                error_box = messagebox.showerror("Instructions", "Please do any kind of plot")
        else:
        ...
        if len(self.filePaths) >= 1:
            if len(self.x_arr) > 0:
                if self.num_cols != 3:
                    ...
                    # Replot button
                    btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_xyticks())
                    btn_replot.place(x=72, y=5)
                else:
                    btn_replot = Button(window, text="Replot", height=1, width=5+self.p)
                    btn_replot.place(x=72, y=5)
            else:
                error_box = messagebox.showerror("Instructions", "Please do any kind of plot")
        else:
    ...
    def set_color(self):
        if self.num_cols == 6 or self.num_cols == 2.5:
            ...
            if len(self.x_arr) > 0:
                ...
                # Replot button
                btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_color())
                btn_replot.place(x=72, y=5)
            else:
                error_box = messagebox.showerror("Instructions", "Please do any kind of plot")
        else:
    ...
    def set_legend(self):
        if len(self.filePaths) >= 1:
            if len(self.x_arr) > 0:
                ...
                # Replot button
                btn_replot = Button(window, text="Replot", height=1, width=5+self.p, command=lambda: self.plot_legend())
                btn_replot.place(x=72, y=5)
            else:
                error_box = messagebox.showerror("Instructions", "Please do any kind of plot")
        else:
    ...
    def set_back(self):
        if self.num_cols == 2:
            ...
            self.back_to_plot3d_file()
        elif self.num_cols == 5:
            self.back_to_lp_file()
        elif self.num_cols == 4:
            self.back_to_scatter_file()
        elif self.num_cols == 6:
            self.back_to_contour_file()
...
def windows_view():
    p = 3
    ps = plot_save(window, p)
    # Menubar
    btn_upload = Button(window, text="Upload", height=1, width=7, command=lambda: ps.upload_file())
    btn_upload.place(x=5, y=5)
    btn_plot = Button(window, text="Line plot", height=1, width=8, command=lambda: ps.plot_file())
    btn_plot.place(x=72, y=5)
    btn_lp = Button(window, text="Line-point", height=1, width=10, command=lambda: ps.lp_file())
    btn_lp.place(x=145, y=5)
    btn_set_xyscatter = Button(window, text="Scatter plot", height=1, width=10, command=lambda: ps.scatter_file())
    btn_set_xyscatter.place(x=235, y=5)
    btn_hist = Button(window, text="Histogram plot", height=1, width=13, command=lambda: ps.set_hist())
    btn_hist.place(x=325, y=5)
    btn_colorbar = Button(window, text="Color z-axis", height=1, width=10, command=lambda: ps.set_colorbar())
    btn_colorbar.place(x=440, y=5)
    btn_3dplot = Button(window, text="3D plot", height=1, width=8, command=lambda: ps.plot3d())
    btn_3dplot.place(x=530, y=5)
    btn_contour = Button(window, text="Contour", height=1, width=8, command=lambda: ps.set_contour())
    btn_contour.place(x=605, y=5)
    btn_save = Button(window, text="Save", height=1, width=6, command=lambda: ps.save_fig())
    btn_save.place(x=680, y=5)

    btn_set_xylabel = Button(window, text="Set label", height=1, width=10, command=lambda: ps.set_xylabel())
    btn_set_xylabel.place(x=5, y=40)
    btn_set_xyrange = Button(window, text="Set range", height=1, width=10, command=lambda: ps.set_xyrange())
    btn_set_xyrange.place(x=95, y=40)
    btn_set_xyfigsize = Button(window, text="Set figsize", height=1, width=10, command=lambda: ps.set_figsize())
    btn_set_xyfigsize.place(x=185, y=40)
    btn_set_xyticks = Button(window, text="Set ticks", height=1, width=10, command=lambda: ps.set_xyticks())
    btn_set_xyticks.place(x=275, y=40)
    btn_set_xycolor = Button(window, text="Set color", height=1, width=8, command=lambda: ps.set_color())
    btn_set_xycolor.place(x=365, y=40)
    btn_set_xylegend = Button(window, text="Set legend", height=1, width=10, command=lambda: ps.set_legend())
    btn_set_xylegend.place(x=440, y=40)
    btn_set_marker = Button(window, text="Set marker", height=1, width=10, command=lambda: ps.set_lp())
    btn_set_marker.place(x=530, y=40)
    input_lbl = Label(window, text="Input section", font=('Arial', 15), bg="white")
    input_lbl.place(x=5, y=320)

    exit_button = Button(window, text="Exit", width=6, command=lambda: window.destroy)
    exit_button.place(x=330, y=500)
    back_button = Button(window, text="Back", width=6, command=lambda: ps.set_back())
...
def linux_view():
    ...
    btn_3dplot.place(x=530, y=5)
    btn_contour = Button(window, text="Contour", height=1, width=5, command=lambda: ps.set_contour())
    btn_contour.place(x=605, y=5)
...
btn_windows = Button(window, text="Windows", height=1, width=7, command=lambda: windows_view())
btn_linux = Button(window, text="Linux", height=1, width=7, command=lambda: linux_view())
btn_windows.place(x=300, y=300)
btn_linux.place(x=400, y=300)
